 ...
 * booking with a commission stamped on it (Stage 2's webhook), grouped
 * by agent -- since a real payout is one bank transfer covering
 * everything an agent is owed, not booking-by-booking, the primary
...
export default async function AdminCommissionsPage({
  ...
  for (const row of rows) {
    const agent = row.sales_agents;
    if (!agent) continue; // shouldn't happen (FK), but keeps the reduce safe
    ...
    <div>
      ...
      {groups.length === 0 && (
        <p className="mt-6 rounded-lg border border-sand-deep bg-white p-4 text-sm text-ink-soft">
          No commission-bearing bookings yet.
        </p>
      )}
