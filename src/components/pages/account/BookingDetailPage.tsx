...
import { PICKUP_CHANGE_CUTOFF_HOURS, type Booking, type Traveler } from "@/lib/bookings/types";
import { whatsappLink } from "@/lib/contact";
import {
  resendConfirmationEmailAction,
  changePickupTimeAction,
  writeReviewAction,
} from "@/app/account/booking/[id]/actions";
import { lombokDateString } from "@/lib/timezone";
import { requestGiftVoucherRefundAction } from "@/app/account/bookings/actions";
import { sendCustomerMessageAction } from "@/app/account/booking/[id]/chat-actions";
...
import type { CancellationRequest, GiftVoucher } from "@/lib/cancellations/types";
...
/**
 * Shared by src/app/account/booking/[id]/page.tsx (English) and
 * src/app/id/account/booking/[id]/page.tsx (Indonesian). Spec §6h
 * booking detail page. Built here: trip info, contact info, the "view
 * confirmation email" action for upcoming bookings, the manual-
 * confirmation flow's status-specific states (§6b) -- under review,
 * confirmed-with-payment-link-and-deadline, declined (with reason),
 * and the traveler/passport-received list -- the per-booking chat
 * thread (§6b/§6c), the cancellation/reschedule request flow (§6f),
 * and the "write a review" entry point (§6d/§6h) once a completed
 * trip's service_end_date has passed.
 */
export async function BookingDetailPage({
  ...
  const customer = await requireCustomer(`${pathPrefix}/account/booking/${id}`);
  const dict = getDictionary(locale).account.booking;
  const chatDict = getDictionary(locale).account.chat;
  ...
  return (
    <div className="max-w-xl">
      <Link href={`${pathPrefix}/account/bookings`} className="text-sm font-semibold text-teal hover:underline">
        {dict.backToMyBookings}
      </Link>
      ...
      <p className="mt-1 text-sm text-ink-soft">
        {dict.bookingCode} <span className="font-semibold text-ink">{b.booking_code}</span> ·{" "}
        {dict.bookingStatus[b.status]}
      </p>
      ...
      {b.status === "under_review" && (
        <p className="mt-4 rounded-lg border border-sand-deep bg-white p-4 text-sm text-ink-soft">
          {dict.underReviewNotice}
        </p>
      )}

      {b.status === "confirmed_awaiting_payment" && (
        <div className="mt-4 rounded-lg border border-teal bg-[#E3F2F1] p-4 text-sm text-teal">
          <p>{dict.awaitingPaymentNotice}</p>
          {b.confirmation_deadline && (
            <p className="mt-1 font-semibold">
              {dict.paymentDueBy(new Date(b.confirmation_deadline).toLocaleString())}
            </p>
          )}
        </div>
      )}
      ...
      {b.status === "declined" && (
        <div className="mt-4 rounded-lg border border-coral bg-[#FCE6DD] p-4 text-sm text-coral-dark">
          <p className="font-semibold">{dict.declinedHeading}</p>
          {b.decline_reason && <p className="mt-1">{b.decline_reason}</p>}
          <p className="mt-1">{dict.declinedNothingCharged}</p>
        </div>
      )}
      ...
      <div className="mt-6 rounded-2xl border border-sand-deep bg-white p-6 text-sm">
        <div className="flex justify-between border-b border-sand-deep py-2">
          <span className="text-ink-soft">{dict.date}</span>
          <span className="text-ink">{b.slot_date}</span>
        </div>
        <div className="flex justify-between border-b border-sand-deep py-2">
          <span className="text-ink-soft">{dict.travelers}</span>
          <span className="text-ink">{b.pax_count}</span>
        </div>
        {b.discount_code && b.discount_amount_usd > 0 && (
          <div className="flex justify-between border-b border-sand-deep py-2">
            <span className="text-ink-soft">{dict.discount(b.discount_code)}</span>
            <span className="text-teal">-{formatUsd(b.discount_amount_usd)}</span>
          </div>
        )}
        {b.insurance_total_idr > 0 && (
          <div className="flex justify-between border-b border-sand-deep py-2">
            <span className="text-ink-soft">{dict.parkInsurance}</span>
            <span className="text-ink">{formatIdr(b.insurance_total_idr)}</span>
          </div>
        )}
        <div className="flex justify-between py-2">
          <span className="text-ink-soft">{b.status === "paid_confirmed" ? dict.totalPaid : dict.total}</span>
      ...
      {isManualConfirmation && travelers.length > 0 && (
        <div className="mt-6 rounded-2xl border border-sand-deep bg-white p-6 text-sm">
          <p className="font-semibold text-ink">{dict.travelersHeading}</p>
            ...
            {travelers.map((t) => (
              <div key={t.id} className="flex items-center justify-between border-t border-sand-deep pt-2 first:border-t-0 first:pt-0">
                ...
                <span className="text-xs text-ink-soft">
                  {t.insurance_type === "park_provided" ? dict.parkInsuranceShort : dict.ownInsurance} ·{" "}
                  {t.passport_scan_path ? dict.passportReceived : dict.passportNotReceived}
                </span>
              </div>
            ))}
          </div>
      ...
      <div className="mt-6 rounded-2xl border border-sand-deep bg-white p-6 text-sm">
        <p className="font-semibold text-ink">{dict.contact}</p>
      ...
      {b.pickup_datetime && (
        <div className="mt-6 rounded-2xl border border-sand-deep bg-white p-6 text-sm">
          <p className="font-semibold text-ink">{dict.pickup}</p>
          <p className="mt-1 text-ink">{new Date(b.pickup_datetime).toLocaleString()}</p>
          <p className="text-ink-soft">
            {dict.from}: {[meetingPointName, b.meeting_point_custom].filter(Boolean).join(", ") || "—"}
            {carLabel && ` · ${carLabel}`}
          </p>
          {(dropoffPointName || b.dropoff_location_custom) && (
            <p className="text-ink-soft">
              {dict.to}: {[dropoffPointName, b.dropoff_location_custom].filter(Boolean).join(", ")}
            </p>
          )}
          {b.passenger_name && <p className="text-ink-soft">{dict.passenger(b.passenger_name)}</p>}
          {b.flight_details && <p className="text-ink-soft">{dict.flight(b.flight_details)}</p>}
          {b.pickup_whatsapp_number && (
            <p className="text-ink-soft">{dict.driverWillMessage(b.pickup_whatsapp_number)}</p>
          )}
          ...
          {canChangePickupTime ? (
            <details className="mt-3 border-t border-sand-deep pt-3">
              <summary className="cursor-pointer list-none text-xs font-semibold text-teal hover:underline">
                {dict.changePickupTime}
              </summary>
              <form
                action={changePickupTimeAction.bind(null, b.id, locale)}
      ...
      {(b.hotel_name || b.room_number) && (
        <div className="mt-6 rounded-2xl border border-sand-deep bg-white p-6 text-sm">
          ...
          {b.room_number && <p className="text-ink-soft">{dict.room(b.room_number)}</p>}
        </div>
      )}

      {b.status === "paid_confirmed" && (
        <form action={resendConfirmationEmailAction.bind(null, b.id, locale)} className="mt-6">
          <button
            type="submit"
            className="rounded-lg border border-teal px-4 py-2 text-sm font-semibold text-teal hover:bg-[#E3F2F1]"
          >
            {dict.resendConfirmation}
          </button>
        </form>
      )}
      ...
      {b.status === "paid_confirmed" && serviceEnded && (
        <div className="mt-6 rounded-2xl border border-sand-deep bg-white p-6 text-sm">
          <p className="font-semibold text-ink">{dict.review}</p>
          {review ? (
            <>
              ...
              <p className="mt-2 text-xs text-ink-soft">
                {review.status === "published"
                  ? dict.liveOnProductPage
                  : review.status === "pending_moderation"
                    ? dict.reviewPendingModeration
                    : dict.reviewSubmitted}
              </p>
            </>
          ) : (
            <>
              <p className="mt-1 text-ink-soft">{dict.howWasTrip}</p>
              <form action={writeReviewAction.bind(null, b.id, locale)} className="mt-3">
                <button
                  type="submit"
                  className="rounded-lg border border-teal px-4 py-2 text-sm font-semibold text-teal hover:bg-[#E3F2F1]"
                >
      ...
      {latestCancellationRequest && (
        <div className="mt-6 rounded-2xl border border-sand-deep bg-white p-6 text-sm">
          <div className="flex items-center justify-between">
            <p className="font-semibold text-ink">{dict.cancellationHeading}</p>
            <span className="text-xs font-semibold uppercase text-ink-soft">
              {dict.cancellationStatus[latestCancellationRequest.status]}
            </span>
          </div>

          {latestCancellationRequest.status === "pending_review" && (
            <p className="mt-2 text-ink-soft">
              {dict.reviewingRequest}
              {latestCancellationRequest.preferred_resolution &&
                dict.forPreference(
                  dict.cancellationPreferredResolution[latestCancellationRequest.preferred_resolution]
                )}
              {latestCancellationRequest.preferred_new_date &&
                dict.newDateRequested(latestCancellationRequest.preferred_new_date)}
              {dict.wellEmailWhenDecided}
            </p>
          )}
          ...
          {latestCancellationRequest.status === "approved" &&
            latestCancellationRequest.resolution === "refund" && (
              <p className="mt-2 text-ink-soft">
                ...
                {dict.isBeingProcessed}
              </p>
            )}

          {latestCancellationRequest.status === "approved" &&
            latestCancellationRequest.resolution === "reschedule" && (
              <p className="mt-2 text-ink-soft">{dict.approvedRescheduled(b.slot_date)}</p>
            )}
          ...
          {latestCancellationRequest.status === "approved" &&
            ...
            vouchers[0] && (
              <div className="mt-2 text-ink-soft">
                <p>{dict.approvedVoucher(vouchers[0].recipient_name)}</p>
                ...
                {vouchers[0].status === "issued" && !vouchers[0].cancellation_requested_at && (
                  <details className="mt-2 border-t border-sand-deep pt-2">
                    <summary className="cursor-pointer list-none text-xs font-semibold text-coral-dark hover:underline">
                      {dict.requestRefundOnVoucher}
                    </summary>
                    <form
                      action={requestGiftVoucherRefundAction.bind(null, vouchers[0].id)}
                      className="mt-2 flex flex-col gap-2"
                    >
                      <input type="hidden" name="return_to" value={`${pathPrefix}/account/booking/${b.id}`} />
                      <input type="hidden" name="locale" value={locale} />
                      <textarea
                        name="reason"
                        rows={3}
                        required
                        placeholder={getDictionary(locale).account.bookings.refundReasonPlaceholder}
                ...
                {vouchers[0].cancellation_requested_at && (
                  <p className="mt-2 border-t border-sand-deep pt-2 text-xs text-teal">
                    {getDictionary(locale).account.bookings.refundRequestedOn(
                      new Date(vouchers[0].cancellation_requested_at).toLocaleDateString()
                    )}
                  </p>
                )}
              </div>
      ...
      {canRequestCancellation && (
        <Link
          href={`${pathPrefix}/account/booking/${b.id}/cancel`}
          className="mt-6 inline-block rounded-lg border border-sand-deep px-4 py-2 text-sm font-semibold text-ink hover:bg-sand"
        >
          {dict.canRequestCancellation}
        </Link>
      )}

      <div className="mt-6 rounded-2xl border border-sand-deep bg-white">
        <div className="border-b border-sand-deep p-4">
          <p className="font-semibold text-ink">{dict.messageUsAboutTrip}</p>
          <p className="text-xs text-ink-soft">{dict.questionsLandHere}</p>
        </div>
        <ChatThread messages={messages} viewerRole="customer" emptyStateText={chatDict.noMessagesYet} />
        <ChatComposer
          action={sendCustomerMessageAction.bind(null, b.id, locale)}
          placeholder={chatDict.writeMessage}
          sendLabel={chatDict.send}
        />
      </div>
